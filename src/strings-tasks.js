...
function removeFirstOccurrences(str, value) {
  ...
  const after = str.slice(firstIndex + value.length);
...
function isPalindrome(str) {
  ...
  return baseStr.split('').reverse().join('') === baseStr;
}
...
function encodeToRot13(str) {
  ...
  const convertCharToRot13 = (char, index) => {
    const direction = index >= 13 ? -1 : 1;

    const rot13 = alphabet[index + 13 * direction];

    return char.toUpperCase() === char
      ? rot13.toUpperCase()
      : rot13.toLocaleLowerCase();
  };
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (indexInAlphabet === -1) {
      ...
      result += convertCharToRot13(char, indexInAlphabet);
    }
  }
